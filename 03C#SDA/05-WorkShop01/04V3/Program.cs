...
namespace _04V3
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    if (!visited[i, j])
                    {
                        int area = Dfs(i, j, graph, visited);
                        if (largestArea < area)
                        {
                            largestArea = area;
                        }
        ...
        private static int Dfs(int row, int col, int[,] graph, bool[,] visited)
        {
            int count = 0;
        ...
        private static int DfsNew(int row, int col, int[,] graph, bool[,] visited)
        {
            int count = 0;

            Node start = new Node() { Row = row, Col = col };
            ...
            while (st.Count > 0)
            {
                ...
                foreach (var child in GetNeighbours(node, graph))
                {
